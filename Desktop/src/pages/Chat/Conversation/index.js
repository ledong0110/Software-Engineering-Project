...
function Conversation(props) {
  ...
  useEffect(() => {
    if (userDetails && selectedUser) {
      (async () => {
        const conversationsResponse = await getConversationBetweenUsers(selectedUser.userID);
        updateMessageLoading(false)
        if (conversationsResponse) {
          updateConversation(conversationsResponse);
        } else if (conversationsResponse === null) {
          updateConversation([]);
        }
      })();
    }
